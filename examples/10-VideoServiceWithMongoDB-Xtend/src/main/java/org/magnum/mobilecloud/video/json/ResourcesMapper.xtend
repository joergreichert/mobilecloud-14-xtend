...
public class ResourcesMapper extends ObjectMapper {
	...
	private val serializer = new JsonSerializer<Resources<?>>() {

		// We are going to register this class to handle all instances of type
		// Resources
		override handledType() {
			(new Resources{} as Resources<?>).class as Class<Resources<?>>
// doesn't work
//			Resources as Class<Resources<?>>
		}

		override serialize(Resources<?> value, JsonGenerator jgen,
			...
			provider.findValueSerializer(content.class, null) => [
				serialize(content, jgen, provider)
			]
		}
	}
